...
2.0.0
```
**注意**：版本号格式必须与 `updater_config.json` 中的 `current` 版本一致。
...
#### 4.3 包含版本信息文件
确保Release中包含 `version.txt` 文件，内容与程序版本一致。
...
# 4. 检查版本号格式
# 确保 version.txt 和 updater_config.json 中版本号一致
```
